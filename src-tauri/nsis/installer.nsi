; Custom NSIS template for ShalomFlow with portable mode support.
...
Function .onInit
  ...
  ; legacy "SpeakoFlow Portable Mode" marker written by installs created before
  ; the ShalomFlow rebrand, the older "Handy Portable Mode" marker written
  ; before the SpeakoFlow rebrand, or the legacy empty marker created by even
  ; older Handy releases. Require Data/ for the legacy empty-marker case so
  ; stale scoop side-effect files do not accidentally opt an updater run into
  ; portable mode.
  ${If} $PortableMode <> 1
  ${AndIf} $UpdateMode = 1
  ${AndIf} ${FileExists} "$INSTDIR\portable"
    ...
    FileRead $1 $2
    FileClose $1
    ${If} $2 == "ShalomFlow Portable Mode"
      StrCpy $PortableMode 1
    ${OrIf} $2 == "SpeakoFlow Portable Mode"
      StrCpy $PortableMode 1
...
Section Install
  ...
  ${If} $PortableMode = 1
    ...
    FileWrite $0 "ShalomFlow Portable Mode"
    FileClose $0
